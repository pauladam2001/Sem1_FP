...
class userInterface:
    ...
    def display_clients(self):
        ...
        for client in range(len(self._clientServiceUI.clientList)):
            print(
                'Client id: ' + str(self._clientServiceUI.clientList[client]['Client id']).rjust(4) + ', Name: ' + str(
                    self._clientServiceUI.clientList[client]['Name']).rjust(4))
    ...
    def display_books(self):
        ...
        for book in range(len(self._bookServiceUI.bookList)):
            print('Book id: ' + str(self._bookServiceUI.bookList[book]['Book id']).rjust(4) + ', Title: ' + str(
                self._bookServiceUI.bookList[book]['Title']).rjust(24) +
    ...
    def display_rentals(self):
        ...
        for rental in range(len(self._rentalServiceUI.rentalList)):
            print('Rental id: ' + str(self._rentalServiceUI.rentalList[rental]['Rental id']).rjust(
                4) + ', Book id: ' + str(self._rentalServiceUI.rentalList[rental]['Book id']).rjust(4) +
                  ', Client id: ' + str(self._rentalServiceUI.rentalList[rental]['Client id']).rjust(
                4) + ', Rented date: ' + str(self._rentalServiceUI.rentalList[rental]['Rented date']).rjust(4) +
    ...
    def return_book(self):
        ...
        if self._rentalServiceUI.check_available_book(
                bookForReturnID):  # if the function returns true, then the book is not rented yet
            raise RentalException('The book is not rented yet so it can not be returned!')
        if not self._rentalServiceUI.check_book_not_returned(
                bookForReturnID):  # if the function returns false, the book is already returned
        ...
        if not self._rentalServiceUI.check_rent_before_return_date(bookForReturnID,
                                                                   returnedDate):  # if the functions returns false, the return date is before the rent date
    ...
    def start_menu_ui(self):
        ...
        menuItems = {'1': self.add_client, '2': self.remove_client, '3': self.update_client, '4': self.display_clients,
                     '5': self.add_book,
                     '6': self.remove_book, '7': self.update_book, '8': self.display_books, '9': self.rent_book,
                     '10': self.return_book,
                     '11': self.display_rentals, '12': self.search_clients, '13': self.search_books,
                     '14': self.most_rented_books,
                     '15': self.most_active_clients, '16': self.most_rented_author, '17': self.undo_command,
                     '18': self.redo_command}
