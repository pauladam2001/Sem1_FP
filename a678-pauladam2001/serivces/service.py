...
class BookService:
    ...
    def add_book_record_history(self, bookID, bookTitle, bookAuthor):
        ...
        operation = self._undoService.create_new_add_operation(self.remove_book, [bookID], self.add_book, [bookID, bookTitle, bookAuthor])
    ...
    def remove_book_record_history(self, bookID, rentalService):  # we need the rental service in order to delete the rentals that have the book
        """
        ...
        bookOperation = self._undoService.create_new_remove_operation(self.add_book, [book.book_id, book.title, book.author], self.remove_book, [bookID])
    ...
    def update_book_record_history(self, bookID, newBookTitle, newBookAuthor):
        ...
        operation = self._undoService.create_new_update_operation(self.update_book, [bookID, title, author], [bookID, newBookTitle, newBookAuthor])
        self._undoService.record(operation)
    ...
    def search_book_id_service(self, bookID):
        ...
        for index in range(len(self._bookRepository.bookList)):
            ...
            if matchingID:
                foundBooksList.append('Book id: ' + str(self._bookRepository.bookList[index]['Book id']).rjust(4) + ', Title: ' + str(self._bookRepository.bookList[index]['Title']).rjust(24) + ', Author: ' + str(self._bookRepository.bookList[index]['Author']).rjust(4))
    ...
    def search_book_author_service(self, bookAuthor):
        ...
        foundBookList = []
        for index in range(len(self._bookRepository.bookList)):
            matchingAuthor = re.search(bookAuthor.strip().lower(), self._bookRepository.bookList[index]['Author'].lower())
            if matchingAuthor:
                foundBookList.append('Book id: ' + str(self._bookRepository.bookList[index]['Book id']).rjust(4) + ', Title: ' + str(self._bookRepository.bookList[index]['Title']).rjust(24) + ', Author: ' + str(self._bookRepository.bookList[index]['Author']).rjust(4))
...
class ClientService:
    ...
    def add_client_record_history(self, clientID, clientName):
        ...
        operation = self._undoService.create_new_add_operation(self.remove_client, [clientID], self.add_client, [clientID, clientName])
        self._undoService.record(operation)
    ...
    def update_client_record_history(self, clientID, newName):
        ...
        operation = self._undoService.create_new_update_operation(self.update_client, [clientID, name], [clientID, newName])
        self._undoService.record(operation)
    ...
    def search_client_id_service(self, clientID):
        ...
        for index in range(len(self._clientRepository.clientList)):
            ...
            if matchingID:
                foundClientsList.append('Client id: ' + str(self._clientRepository.clientList[index]['Client id']).rjust(4) + ', Name: ' + str(self._clientRepository.clientList[index]['Name']).rjust(4))
    ...
    def search_client_name_service(self, clientName):
        ...
        foundClientsList = []
        for index in range(len(self._clientRepository.clientList)):
            matchingName = re.search(clientName.strip().lower(), self._clientRepository.clientList[index]['Name'].lower())
            if matchingName:
                foundClientsList.append('Client id: ' + str(self._clientRepository.clientList[index]['Client id']).rjust(4) + ', Name: ' + str(self._clientRepository.clientList[index]['Name']).rjust(4))
...
class RentalService:
    ...
    """
    def __init__(self, rentalRepository, bookService, clientService, undoService):
        self._rentalRepository = rentalRepository
    ...
    def check_available_book(self, bookForRentID):
        ...
        for index in range(len(self._rentalRepository.rentalList)):
            if bookForRentID == self._rentalRepository.rentalList[index]['Book id'] and self._rentalRepository.rentalList[index]['Returned date'] == '':
                return False
        return True
    ...
    def update_return_date_record_history(self, bookForReturnID, newReturnedDate):
        ...
        operation = self._undoService.create_new_update_operation(self.update_return_date, [bookForReturnID, returnedDate], [bookForReturnID, newReturnedDate])
        self._undoService.record(operation)
    ...
    def add_rental_record_history(self, rentalID, bookForRentID, clientRentID, rentedDate, returnedDate):
        ...
        operation = self._undoService.create_new_add_operation(self.remove_rental, [rentalID], self.add_rental, [rentalID, bookForRentID, clientRentID, rentedDate, returnedDate])
        self._undoService.record(operation)
    ...
    def remove_by_book_id(self, bookID):
        ...
        while index < len(self._rentalRepository.rentalList):
            if bookID == self._rentalRepository.rentalList[index]['Book id']:
                rental = Rental(self._rentalRepository.rentalList[index]['Rental id'], self._rentalRepository.rentalList[index]['Book id'], self._rentalRepository.rentalList[index]['Client id'], self._rentalRepository.rentalList[index]['Rented date'], self._rentalRepository.rentalList[index]['Returned date'])
                self._rentalRepository.remove(self._rentalRepository.rentalList[index]['Rental id'])
                operation = self._undoService.create_new_remove_operation(self.add_rental, [rental.rental_id, rental.book_id_rental, rental.client_id_rental, rental.rented_date, rental.returned_date], self.remove_rental, [rental.rental_id])
    ...
    def remove_by_client_id(self, clientID):
        ...
        while index < len(self._rentalRepository._rentalList):
            if clientID == self._rentalRepository._rentalList[index]['Client id']:
                rental = Rental(self._rentalRepository._rentalList[index]['Rental id'], self._rentalRepository._rentalList[index]['Book id'], clientID, self._rentalRepository._rentalList[index]['Rented date'], self._rentalRepository._rentalList[index]['Returned date'])
                self._rentalRepository.remove(self._rentalRepository.rentalList[index]['Rental id'])
                operation = self._undoService.create_new_remove_operation(self.add_rental, [rental.rental_id, rental.book_id_rental, rental.client_id_rental, rental.rented_date, rental.returned_date], self.remove_rental, [rental.rental_id])
                allOperations += (operation,)
            else:
    ...
    def most_rented_books(self, bookList):
        ...
        for rental in range(len(self._rentalRepository.rentalList)):                               #we store in the dictionary how many times each rented book was rented
        ...
        for bookID in rentedBooksDictionary:
            bookPosition = self._bookService.check_book_in_list(bookID)  # we need the position where the book is in the bookList to obtain the title and the author
            sortedListOfRentedBooks.append(BookRentalTimes(bookID, bookList[bookPosition]['Title'], bookList[bookPosition]['Author'], rentedBooksDictionary[bookID]))
    ...
    def number_of_rented_days(self, clientPosition):
        ...
        if self._rentalRepository.rentalList[clientPosition]['Returned date'] != '':
            return (self._rentalRepository.rentalList[clientPosition]['Returned date'] - self._rentalRepository.rentalList[clientPosition]['Rented date']).days  #without .days the hours will be displayed too
    ...
    def most_active_clients(self, clientList):
        ...
        for rental in range(len(self._rentalRepository.rentalList)):
            if self._rentalRepository.rentalList[rental]['Client id'] not in activeClientsDictionary:
                activeClientsDictionary[self._rentalRepository.rentalList[rental]['Client id']] = self.number_of_rented_days(rental)
            else:
                activeClientsDictionary[self._rentalRepository.rentalList[rental]['Client id']] += self.number_of_rented_days(rental)

        for clientID in activeClientsDictionary:
            clientPosition = self._clientService.check_client_in_list(clientID)         #we need the position in order to know the name
            sortedListOfClients.append(ClientRentalDays(clientID, clientList[clientPosition]['Name'], activeClientsDictionary[clientID]))
    ...
    def most_rented_authors(self, bookList):
        ...
        for rental in range(len(self._rentalRepository.rentalList)):
            bookPosition = self._bookService.check_book_in_list(self._rentalRepository.rentalList[rental]['Book id'])   #we need the position in order to know the author
...
class ClientRentalDays:
    ...
    def __str__(self):
        return 'Client id: ' + str(self.client_id).rjust(4) + ', Name: ' + str(self.client_name).rjust(24) + ', Number of rental days: ' + str(self.client_rental_days)
